...
export default class GDSource extends GuildComponent implements AudioSource {
	...
	async bufferStream(attempts?: number) {
		...
		if (this._gdPCMConverterOutput) { this._gdPCMConverterOutput.removeAllListeners('end'); }
		if (this._gdPCMConverter) { this._gdPCMConverter.kill('SIGINT'); }
	...
	async destroy() {
		...
		if (this._gdPCMConverter) { this._gdPCMConverter.kill('SIGINT'); }
		if (this._gdPCMConverterInput) { this._gdPCMConverterInput.end(); }
